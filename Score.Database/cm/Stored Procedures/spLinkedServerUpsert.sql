...
AS 
	...
	USING (SELECT @Name, @DataSource, @Catalog, @ProductName, @Provider, @ProviderString, @DistPublisher, @Distributor, @Publisher, @Subscriber, @Rpc, @RpcOut, @DataAccess, @DateLastModified, @State, @Active, @dbLastUpdate, @dbLastUpdate) 
		AS source 
		([Name], [DataSource], [Catalog], [ProductName], [Provider], [ProviderString], [DistPublisher], [Distributor], [Publisher], [Subscriber], [Rpc], [RpcOut], [DataAccess], [DateLastModified], [State], [Active], [dbAddDate], [dbLastUpdate])
	-- !!!! Check the criteria for match
	ON ([DatabaseInstanceGUID] = @DatabaseInstanceGUID AND [ID] = @ID)
       WHEN MATCHED THEN 
		UPDATE 
		SET    [Name] = @Name, [DataSource] = @DataSource, [Catalog] = @Catalog, [ProductName] = @ProductName, [Provider] = @Provider, [ProviderString] = @ProviderString, [DistPublisher] = @DistPublisher, [Distributor] = @Distributor, [Publisher] = @Publisher, [Subscriber] = @Subscriber, [Rpc] = @Rpc, [RpcOut] = @RpcOut, [DataAccess] = @DataAccess, [DateLastModified] = @DateLastModified, [State] = @State, [Active] = @Active, [dbLastUpdate] = @dbLastUpdate
	WHEN NOT MATCHED THEN
		INSERT ([DatabaseInstanceGUID], [ID], [Name], [DataSource], [Catalog], [ProductName], [Provider], [ProviderString], [DistPublisher], [Distributor], [Publisher], [Subscriber], [Rpc], [RpcOut], [DataAccess], [DateLastModified], [State], [Active], [dbAddDate], [dbLastUpdate])
