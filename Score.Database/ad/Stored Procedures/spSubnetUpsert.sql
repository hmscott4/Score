...
AS 
	...
	-- TEST FOR UNIQUENESS
	IF EXISTS (SELECT [DistinguishedName] FROM [ad].[Subnet] WHERE [DistinguishedName] = @DistinguishedName AND [objectGUID] != @objectGUID)
	BEGIN
	...
	USING (SELECT @Domain, @Name, @Description, @Location, @Site, @DistinguishedName, @whenCreated, @whenChanged, @Active, @dbAddDate, @dbLastUpdate) 
		AS source 
		([Domain], [Name], [Description], [Location], [Site], [DistinguishedName], [whenCreated], [whenChanged], [Active], [dbAddDate], [dbLastUpdate])
	...
	WHEN NOT MATCHED THEN
		...
		VALUES (@objectGUID, @Domain, @Name, @Description, @Location, @Site, @DistinguishedName, @whenCreated, @whenChanged, @Active, @dbAddDate, @dbLastUpdate)
	;
